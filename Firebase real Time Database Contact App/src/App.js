...
import "firebase/database"; //as i am going to use firebase database
import "firebase/storage";  //as i am going to use firebase storage
...
const App = () => {
  ...
  const getContacts = async () => {
    // TODO: load existing data.... grab all data from firebase before anything loads up
    ...
    const contactsRef = await firebase.database().ref("/contacts"); //grab contacts reference from firebase 
    contactsRef.on("value", snapshot =>{   // from firebase anything u get is known as snapshot.we can name it anything
      dispatch({
        type: SET_CONTACT,
